...
const IndexPage = () => {
  ...
  return (
    <View className="flex flex-col h-full bg-background">
      ...
      {/* 商品网格 */}
      <View className="flex-1 px-4 py-3 overflow-y-auto">
        <View className="flex flex-row items-center justify-between mb-3">
          <Text className="block text-base font-semibold text-gray-800">精选商品</Text>
          <View className="flex flex-row items-center" onClick={goToHot}>
            <Text className="block text-xs text-gray-400 bg-gray-100 rounded-full px-3 py-1">更多</Text>
          </View>
        </View>
        ...
        {loading ? (
          ...
          <View className="grid grid-cols-2 gap-3">
            {products.map((product) => (
              ...
              >
                <View className="bg-gradient-to-br from-gray-100 to-gray-200 h-28 rounded-xl mb-2 flex items-center justify-center overflow-hidden">
                  {product.image_url ? (
                    <Image
                ...
                <View className="flex items-center justify-between mt-1">
                  ...
                  {product.is_hot && (
                    <View className="bg-red-50 rounded-full px-2 py-1">
                      <Text className="block text-xs text-red-500">热销</Text>
                    </View>
                  )}
                </View>
              </View>
